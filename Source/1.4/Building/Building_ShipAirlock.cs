...
namespace RimWorld
{
    public class Building_ShipAirlock : Building_Door
    {
		...
		public override bool PawnCanOpen(Pawn p)
        {
            ...
            {
                if (!(ShipInteriorMod2.ExposedToOutside(p.GetRoom()) || (ShipInteriorMod2.EVAlevel(p)>3 && (!mapComp.InCombat || p.Map.mapPawns.AnyColonistSpawned)) || p.CurJobDef == ResourceBank.JobDefOf.FleeVacuum))
                    return false;
            }
            Lord lord = p.GetLord();
