	...
	public class CompShuttleLaunchable : ThingComp
	{
		...
		public override IEnumerable<Gizmo> CompGetGizmosExtra()
		{
			...
			if (this.LoadingInProgressOrReadyToLaunch)
			{
				...
				if (this.parent.Map.Parent is WorldObjectOrbitingShip && this.parent.Map.GetComponent<ShipMapComp>().ShipMapState == ShipMapState.inCombat)
				{
		...
		{
			ChoseWorldTarget(this.parent.Map.GetComponent<ShipMapComp>().ShipCombatTargetMap.Parent);
		}
		...
		private void Launch(GlobalTargetInfo target, TransportPodsArrivalAction arrivalAction, bool single = false)
		{
			...
			for (int i = 0; i < transportersInGroup.Count; i++)
			{
				...
				if (single && parent.TryGetComp<CompShuttleCosmetics>() != null)
				{
					...
					CompProps_ShuttleCosmetics Props = parent.TryGetComp<CompShuttleCosmetics>().Props;
